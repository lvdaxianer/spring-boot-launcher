...
- 专注磁盘存储：优化本地文件操作，性能优异
- 完善的异常体系：统一的错误码管理，便于问题排查
- 丰富的日志支持：可配置的日志级别和动态日志输出
...
io.lvdaxianer.upload.file.log-level=DEBUG
...
1. **路径安全**：组件已内置路径遍历攻击防护，不支持绝对路径
2. **并发安全**：多线程上传同一文件时，分片文件会相互覆盖，需确保唯一标识
3. **目录清理**：文件合并成功后，临时分片文件会自动删除
